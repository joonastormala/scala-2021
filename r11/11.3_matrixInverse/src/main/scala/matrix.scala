...
/* 
 * This is 'matrix.scala'. 
 * See "Task 1" below for your task.
 * (Task 2 is in 'leastSquares.scala')
 *
 */
...
class Matrix(val m: Int, val n: Int) {
  ...
  override def toString = {
    ...
    if(m <= 6 && n <= 6) {
      ...
      s ++= "[%d-by-%d matrix -- output suppressed]".format(m,n)
    }
    s.toString
  }
   ...
   * Task 1: 
   *
   * Implement the following method that returns the multiplicative 
   * inverse of this matrix. For example, you can use Gaussian elimination.
   *
   * Remark:
   * You may assume the matrix to be invertible and numerically well-behaved.
   *
   */
  def inverse: Matrix = {
    require(n == m)
