...
package object sequences {
  ...
  def minOfSquaresAtLeast17OfPositive(in: Seq[Int]): Int =
    in.filter(x => x*x > 16 && x>0).map(x=>x*x).min
  ...
  def squareOfDDimEuclideanDistance(a: Seq[Int], b: Seq[Int]): Int =
    (a zip b).map(x => (x._1-x._2)*(x._1-x._2)).reduce(_+_)
  ...
  def consecutivePositionPairs(z: Seq[Int]): Seq[(Int,Int)] = z.take(z.length-1) zip (z.drop(1))
  ...
  def firstMaxPos(a: Seq[Int]): Int =
    a.zipWithIndex.filter(_._1 == a.max).map(_._2).head
  ...
  def sumAndDifferenceSeqs(a: Seq[Int], b: Seq[Int]): (Seq[Int],Seq[Int]) =
    ((a zip b).map(x=> x._1+x._2) zip (a zip b).map(x=> x._1-x._2)).unzip
  ...
  def movingSum(in: Seq[Int], w: Int): Seq[Int] = {
    (in.scanLeft(0)(_+_).drop(w) zip in.scanLeft(0)(_+_).dropRight(w)).map(x => x._1-x._2)
